...
public class HashJoinAssembly extends BaseComponent<HashJoinEntity> {
	...
	private static Logger LOG = LoggerFactory.getLogger(HashJoinAssembly.class);
	...
	protected void createAssembly() {
		try {
			if (LOG.isTraceEnabled()) {
				LOG.trace(hashJoinEntity.toString());
			}
			for (OutSocket outSocket : hashJoinEntity.getOutSocketList()) {
				LOG.trace("Creating hash join assembly for '"
						+ hashJoinEntity.getComponentId() + "' for socket: '"
	...
	private void prepare(OutSocket outSocket) {
		...
		uniqInputFields = new Fields[componentParameters.getInputFieldsList()
				.size()];
		uniqKeyFields = new Fields[hashJoinEntity.getAllKeyFieldSize()];
		inputPipes = new Pipe[componentParameters.getInputPipes().size()];

		Fields inputFields;
		Fields keyFields = new Fields();

		joiner = new HashJoinJoiner(getMatchOption(hashJoinEntity.getMatch()));

		for (int i = 0; i < componentParameters.getInputPipes().size(); i++) {
			inputFields = componentParameters.getInputFieldsList().get(i);
			int[] inputFieldsPos = inputFields.getPos();
			for (JoinKeyFields keyFieldsEntity : hashJoinEntity.getKeyFields()) {
