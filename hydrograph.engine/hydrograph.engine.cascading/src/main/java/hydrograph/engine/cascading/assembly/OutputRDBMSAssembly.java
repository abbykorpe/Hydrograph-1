...
public abstract class OutputRDBMSAssembly extends BaseComponent {
	...
	protected static Logger LOG = LoggerFactory
			.getLogger(OutputRDBMSAssembly.class);


	public OutputRDBMSAssembly(OutputRDBMSEntity baseComponentEntity,
			ComponentParameters componentParameters) {
		super(baseComponentEntity, componentParameters);
	}

@Override
public void initializeEntity(AssemblyEntityBase assemblyEntityBase) {
...
public abstract void  intializeRdbmsSpecificDrivers() ;
	...
	protected void createTableDescAndScheme() {
		tableDesc = new TableDesc(outputRDBMSEntity.getTableName(),
				columnNames, columnDefs, primaryKeys);
	...
	scheme = new JDBCScheme( inputFormatClass, DBOutputFormat.class, fields, columnNames, null, new Fields(updateByColumns), updateByColumns );
	}
	}
	...
	protected  void initializeRDBMSTap()
	{
		...
		rdbmsTap = new JDBCTap(outputRDBMSEntity.getJdbcurl() + 
				(outputRDBMSEntity.getDatabaseName() == null ? "" : "/"	+ outputRDBMSEntity.getDatabaseName()),
				outputRDBMSEntity.getUsername(), outputRDBMSEntity.getPassword(),
				driverName, tableDesc, scheme, sinkMode);
	...
	protected void prepareAssembly() {		
		...
		LOG.debug( outputRDBMSEntity.getDatabaseType() + " Output Component: [ Database Name: "
				+ outputRDBMSEntity.getDatabaseName() + ", Table Name: "
				+ outputRDBMSEntity.getTableName() + ", Column Names: "
		...
		try {
			...
			LOG.error("Error in preparing scheme for component '"
					+ outputRDBMSEntity.getComponentId() + "': " + e.getMessage());
	...
	protected void createAssembly() {
		...
		fieldsCreator = new InputOutputFieldsAndTypesCreator<OutputRDBMSEntity>(
				outputRDBMSEntity);
		...
		try {
			...
			LOG.error(
					"Error in creating assembly for component '"
							+ outputRDBMSEntity.getComponentId() + "', Error: "
							+ e.getMessage(), e);
			throw new RuntimeException(e);
		}
	}
