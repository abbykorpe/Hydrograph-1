...
class TeradataTableDescriptorTest {
  ...
  def itShouldGenerateCreateQueryWithoutPrimaryKeys(): Unit = {
    ...
    //given
    val databaseType: String = "teradata"
    val tableName = "teradata_table_1"
    ...
    //when
    val createQuery = new TeradataTableDescriptor(tableName, fieldNames, colDefs, primaryKeys, databaseType).getCreateTableStatement()


    //then
    val expectedQuery: String = "CREATE TABLE teradata_table_1 ( \"f1\" VARCHAR(256),\"f2\" INT,\"f3\" DOUBLE PRECISION,\"f4\" DECIMAL(10,2),\"f5\" DATE,\"f6\" BYTEINT,\"f7\" TIMESTAMP )";
  ...
  def itShouldGenerateCreateQueryWithPrimaryKeys(): Unit = {
    ...
    //then
    val expectedQuery:String="CREATE TABLE abc ( \"f1\" VARCHAR(256)  NOT NULL,\"f2\" INT  NOT NULL,\"f3\" DOUBLE PRECISION,\"f4\" DECIMAL(10,2),\"f5\" DATE,\"f6\" BYTEINT,\"f7\" TIMESTAMP ) UNIQUE PRIMARY INDEX( f1,f2 )";
