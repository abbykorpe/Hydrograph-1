...
class OutputFileCsvWithDateFormatsComponentTest {
  ...
  def executedBeforeEachTestCase(): Unit = {
    ...
    outputFileDelimitedEntity.setPath("testData/outputFiles/csvOutput")
  ...
  def itShouldCheckFirstRecordInOutputFileIsHeaderWhenHasHeaderIsTrue() : Unit = {
    ...
    //when
    val comp = new OutputFileCsvWithDateFormatsComponent(outputFileDelimitedEntity, baseComponentParams)
    comp.execute()

    val dataFrame = spark.read.format("text").load("testData/outputFiles/csvOutput")
  ...
  def itShouldCheckFirstRecordInOutputFileIsDataWhenHasHeaderIsFalse() : Unit = {
    ...
    def itShouldWriteFileWithProvidedDelimiter() : Unit = {
      ...
      def itShouldWriteFewColumnProvidedInEntity() : Unit = {
        ...
        outputFileDelimitedEntity1.setPath("testData/outputFiles/csvFewColumnOutput")
        val sf0: SchemaField = new SchemaField("col1", "java.lang.String")
        val sf1: SchemaField = new SchemaField("col9", "java.util.Date")
        sf1.setFieldFormat("yyyy-MM-dd")
        val sf2: SchemaField = new SchemaField("col10", "java.util.Date");
        sf2.setFieldFormat("yyyy-MM-dd HH:mm:ss")
        ...
        //when
        val comp = new OutputFileCsvWithDateFormatsComponent(outputFileDelimitedEntity1, baseComponentParams)
        comp.execute()

        val dataFrame = spark.read.format("text").load("testData/outputFiles/csvFewColumnOutput")

        //then
        val expectedRowData = "[aaa,2014-02-05,2014-02-05 14:25:36]"
        Assert.assertEquals(expectedRowData,dataFrame.collectAsList().get(0).toString())
      }
        ...
        def itShouldWriteQuoteCharacterInOutputFile() : Unit = {
          ...
          outputFileDelimitedEntity2.setPath("testData/outputFiles/csvOutputQuote")
          ...
          //when
          val comp = new OutputFileCsvWithDateFormatsComponent(outputFileDelimitedEntity2, baseComponentParams1)
          comp.execute()

          val dataFrame = spark.read.format("text").load("testData/outputFiles/csvOutputQuote")
