...
// This will append data for insert,newtable and truncateLoad
  ...
  def getDataFrameWriter(hadoopConfiguration: Configuration, dataFrameWriter: DataFrameWriter[Row], properties: Properties): DataFrameWriter[Row] = {
    if (properties.getProperty("aws_iam_role") != null && !properties.getProperty("aws_iam_role").equals("")) {
      dataFrameWriter.option("aws_iam_role", properties.getProperty("aws_iam_role"))
      LOG.debug("aws_iam_role for output spark redshift component '" + properties.getProperty("aws_iam_role") + "'")
    ...
    if (properties.getProperty("forward_spark_s3_credentials") != null && !properties.getProperty("forward_spark_s3_credentials").equals("")) {
      ...
      dataFrameWriter.option("forward_spark_s3_credentials", properties.getProperty("forward_spark_s3_credentials").toBoolean)
      LOG.info("forward_spark_s3_credentials for output spark redshift component '" + properties.getProperty("forward_spark_s3_credentials") + "'")
    }
    if (properties.getProperty("extracopyoptions") != null && !properties.getProperty("extracopyoptions").equals("")) {
      dataFrameWriter.option("extracopyoptions", properties.getProperty("extracopyoptions"))
      LOG.info("extracopyoptions '" + properties.getProperty("extracopyoptions") + "' for output spark redshift component")
    }
    ...
    if ((properties.getProperty("fs.s3n.awsAccessKeyId") != null && !properties.getProperty("fs.s3n.awsAccessKeyId").equals("")) &&
      ...
      LOG.debug("fs.s3n.awsAccessKeyId '" + properties.getProperty("fs.s3n.awsAccessKeyId") + "' and "
       ...
       + "for output spark redshift component")
    }
    dataFrameWriter
  }
