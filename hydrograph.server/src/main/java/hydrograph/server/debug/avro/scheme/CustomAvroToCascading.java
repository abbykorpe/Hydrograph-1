...
package hydrograph.server.debug.avro.scheme;
...
/**
 * Parse avro record and coerce date and bigdecimal types
 * 
 * @author abhishekk
 *
 */
...
public class CustomAvroToCascading {
	 ...
	 * @param record
	 * @param readerSchema
	 * @return an array of type {@link Object}
	 */
	public static Object[] parseRecord(IndexedRecord record, Schema readerSchema) {
		...
		for (int i = 0; i < schemaFields.size(); i++) {
			...
			// result[i] = (readerSchema.getJsonProp("logicalType") != null &&
			// readerSchema
			// .getJsonProp("logicalType").asText()
			// .equalsIgnoreCase(DATE_TYPE_NAME)) ? fromAvroLong(obj,
			// readerSchema.getValueType()) :
			// readerSchema.getJsonProp("logicalType") != null && readerSchema
			// .getJsonProp("logicalType").asText()
			// .equalsIgnoreCase(DATE_TYPE_NAME) ? fromAvroLong(obj,
			// readerSchema) : obj;

			// result[i] = (readerSchema.getType().equals(Type.LONG) ||
			// readerSchema
			// .getType().equals(Type.RECORD)) ? fromAvro(obj,
			// readerSchema.getValueType()) : obj;
...
//			return fromAvroMap(obj, schema);
		...
		case LONG:
			return fromAvroLong(obj, schema);
	...
	protected static Object fromAvroLong(Object object, Schema schema) {
		if (schema.getJsonProp("logicalType") != null
				&& schema.getJsonProp("logicalType").asText()
						.equalsIgnoreCase(DATE_TYPE_NAME)) {
