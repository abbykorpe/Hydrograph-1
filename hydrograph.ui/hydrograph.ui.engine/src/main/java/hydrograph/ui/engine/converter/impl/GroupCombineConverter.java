...
import hydrograph.engine.jaxb.groupcombine.TypeSecondaryKeyFields;
import hydrograph.engine.jaxb.groupcombine.TypeSecondayKeyFieldsAttributes;
...
/**
 * Groupcombine converter
 * 
 * @author Bitwise 
 */

public class GroupCombineConverter extends TransformConverter {
	private static final Logger logger = LogFactory.INSTANCE.getLogger(GroupCombineConverter.class);
	private TransformMapping transformMapping;
	private List<BasicSchemaGridRow> schemaGridRows;

	public GroupCombineConverter(Component component) {
		super(component);
		this.baseComponent = new Groupcombine();
	...
	private void setPrimaryKeys(Groupcombine groupCombine) {
		...
		if (columnNameProperties != null ) {
			groupCombine.setPrimaryKeys(primaryKeyFields);
	...
	private void setSecondaryKeys(Groupcombine groupCombine) {
